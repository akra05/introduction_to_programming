...
Canvas paint_it_black(Canvas c) {
    for(int i = 0;canvas_height(c)-1 >= i;i++){
        for(int j = 0;canvas_width(c)-1>=j;j++){
...
Canvas descending_diagonal(Canvas c) {
      for(int row = canvas_height(c);row >= 0;row--){
        int pixel_in_row = 0;
        for(int row_lenght = (canvas_height(c)-1)-row;row_lenght >= pixel_in_row && pixel_in_row < canvas_width(c);pixel_in_row++){
            c = canvas_set_black(c,pixel_in_row,row);
        }
    }
    return c;
}
...
Canvas draw_rectangle(Canvas c, int x, int y, int width, int height) {
    int y_pos = y;
    if(y_pos > canvas_height(c)-1){
        y_pos = canvas_height(c)-1;   
    }
    for(;y-height < y_pos && y_pos >= 0;y_pos--){
        int x_pos = x;
        if(x_pos < 0){
            x_pos = 0;
        }
        for(;width+x>x_pos && x_pos <= canvas_width(c)-1;x_pos++){
...
Canvas draw_rectangle_via_corners(Canvas c, int x0, int y0, int x1, int y1) {
    int width_ = (x1 - x0)+1;
    int height_ = (y0 - y1)+1;
    draw_rectangle(c,x0,y0,width_,height_);
    return c;
}
