...
*/
void height_after_rotation(HeightTreeNode *y){
    if (y == NULL){
        return;
    }
    int left_height = height_or_zero(y->left);
    int right_height = height_or_zero(y->right);

    if(left_height > right_height){
        y->height = 1+left_height;
    }
    else{
        y->height = 1 +right_height;
    }
...
HeightTreeNode *rotate_right(HeightTreeNode *t) {
    ...
    y->right = t;
    t->parent = y;
    height_after_rotation(t);
    height_after_rotation(y);
    return y;
}
...
Rotations determine_rotations(HeightTreeNode *t) {
    if(t->left == NULL && t->right->left != NULL){
        if(t->right->right != NULL && t->right->left->height > t->right->right->height){
    ...
    if(t->right == NULL && t->left->right != NULL ){
        if(t->left->left != NULL && t->left->right->height > t->left->left->height){
    ...
    if(t->left->height >= t->right->height+2 && t->left->right->height > t->left->left->height){
        return DoubleRotationLeftRight;
    }
    if(t->right->height >= t->left->height+2 && t->right->left->height > t->right->right->height){
        return DoubleRotationRightLeft;
    }
