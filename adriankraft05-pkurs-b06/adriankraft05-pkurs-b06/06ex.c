...
Canvas swatch_red_green(Canvas c) {
    for(int y = 0;y <= canvas_height(c)-1;y++){
        for(int x = 0;x <= canvas_width(c)-1;x++){
            canvas_set_r(c,x,y,normalize(x,canvas_width(c)-1));
            canvas_set_g(c,x,y,normalize(y,canvas_height(c)-1));
        }
    }
    return c;
}
...
Canvas canvas_set_channel(Canvas c, int x, int y, ColorChannel channel, float v) {
    ColorChannel color = channel; //Nochmal ueberpruefen ob Channel bezeichnung mit zahlen richtig ist
    if(color == 0){
...
Canvas swatch_rgb(Canvas c, ColorChannel channel_x, ColorChannel channel_y) {
    for(int y = 0;y <= canvas_height(c)-1;y++){
        ...
        for(int x = 0;x <= canvas_width(c)-1;x++){
            canvas_set_channel(c,x,y,channel_x,normalize(x,canvas_width(c)-1));
            canvas_set_channel(c,x,y,channel_y,normalize(y,canvas_height(c)-1));
        }
    }
...
RGB rgb_to_gray(RGB color) {
    float grey_r = color.r * 0.2989;
    float grey_g = color.g * 0.587;
    float grey_b = color.b * 0.1140;
    float grey_scale = grey_r + grey_g + grey_b;
    RGB grey_color = {grey_scale, grey_scale, grey_scale};
    return grey_color;
}
...
*/
RGB GetColor(Canvas c,int x,int y){
    RGB color = { .r = canvas_get_r(c,x,y), .g = canvas_get_g(c,x,y), .b = canvas_get_b(c,x,y)};
    return color;
}
Canvas canvas_to_gray(Canvas c) {
    for(int y = 0;y<=canvas_height(c)-1;y++){
        for(int x = 0;x<=canvas_width(c)-1;x++){
            RGB color = GetColor(c,x,y);
            RGB grey_color = rgb_to_gray(color);
            canvas_set_r(c,x,y,grey_color.r);
            canvas_set_g(c,x,y,grey_color.g);   
            canvas_set_b(c,x,y,grey_color.b);
