...
bool is_max_heap(Array arr) {  
    ...
    for (int i =0; 2*i+1 < arr.len; i++) { 
        
        if (arr.arr[i] < arr.arr[2*i+1 ]|| (arr.arr[i] < arr.arr[2*i+2] && arr.len > 2*i+2)) {
            return false; 
        }  
...
*/
void recursion(TreeNode *parent, Array arr, int index){
    if (parent != NULL){ 
        recursion(parent->right, arr, 2*index+1);
        recursion(parent->left,arr, 2*index);
        arr.arr[index - 1] = parent->item;
    }
}
