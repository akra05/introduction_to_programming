...
uint32_t *return_pointer(uint32_t x) {
    uint32_t *p = (uint32_t*) malloc(sizeof(x));
    *p = x;
...
PileOfPancakes create_pancake(uint32_t n) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        new_layer->further_layers = NULL;
        current_layer->further_layers = new_layer;
...
uint16_t *create_dynamic_array(size_t x) {
    uint16_t *square_number_array = malloc(x * sizeof(uint16_t));
    
    for(int i = 0;i<x;i++){
         *(square_number_array + i) = (i+1)*(i+1);
    }
    return square_number_array;
}
