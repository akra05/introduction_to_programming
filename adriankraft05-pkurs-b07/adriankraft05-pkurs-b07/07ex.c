...
/*
Aufgabe 1d:
`x` und `y` zeigen beide auf 32-bit Integer. Setzen Sie den Wert auf den `x` zeigt auf den Wert auf den `y` zeigt.
...
int32_t frumpleflabblefizzledipp(int32_t x) {
    ...
    int32_t t = x;
    fizzledipp(p);
    int32_t flabble_result = flabble(&t, p);
    int32_t z = 0;
    int32_t buffer = 27;
    frumple(flabble_result, &buffer, &z);
...
int32_t count_fruit_layers(PileOfPancakes p) {
    int32_t fruit_count = 0;
    if(p.layer == Fruit){
        fruit_count  += 1;
    }
    if (p.further_layers != 0) {
        fruit_count += count_fruit_layers(*p.further_layers);
    }
