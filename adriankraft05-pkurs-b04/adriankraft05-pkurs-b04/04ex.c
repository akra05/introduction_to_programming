...
Canvas draw_odd_circle(Canvas c, int x, int y, int radius_from_middle) {
    for(int y_pos = y + radius_from_middle; y_pos >= y-radius_from_middle;y_pos--){

        for(int x_pos = x - radius_from_middle;x_pos <=  x + radius_from_middle;x_pos++){

            if(0 <= y_pos && y_pos < canvas_height(c)){

                if(0 <= x_pos && x_pos < canvas_width(c)){

                    if(distance(x,y,x_pos,y_pos) <= radius_from_middle){
                        c = canvas_set_black(c,x_pos,y_pos);
                    }
                }
            }
...
int my_distance(int x0, int y0, int x1, int y1) {
    ...
    int distance = squareroot((x_distance*x_distance)+(y_distance*y_distance));
    return distance ;
}
...
Canvas draw_circle(Canvas c, int x, int y, int diameter) {
    int x_midpoint = x + diameter/2;
    int y_midpoint = y + diameter/2;
    if(diameter%2!=0){
        draw_odd_circle(c,x_midpoint,y_midpoint,diameter/2);
    }
    else{
        for(int i = 0;i <= 1;i++){
            for(int j = 0;j <= 1;j++){
                draw_odd_circle(c,x_midpoint + i-1,y_midpoint + j-1,(diameter/2)-1);
            }
        }
    }
