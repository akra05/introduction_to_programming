...
Canvas sierpinski_carpet(Canvas c, int n, int x, int y){
    if (n == 0) {
        ...
        int distance = power(3, n - 1);
        sierpinski_carpet(c, n - 1, x , y);
        sierpinski_carpet(c, n - 1, x + distance , y);
        sierpinski_carpet(c, n - 1, x + 2 * distance , y);
        sierpinski_carpet(c, n - 1, x, y + distance);
        sierpinski_carpet(c, n - 1, x + 2 * distance, y + distance);
        sierpinski_carpet(c, n - 1, x , y + 2 * distance);
        sierpinski_carpet(c, n - 1, x + distance , y + 2 * distance);
        sierpinski_carpet(c, n - 1, x + 2 * distance, y + 2 * distance);
    }
...
Canvas bucket_fill(Canvas c, int x, int y) {
    ...
    if (x >= 0 && x < canvas_width(c) && y >= 0 && y < canvas_height(c) && !pixel_is_black(c, x, y)) {
        canvas_set_black(c, x, y);
        canvas_save_debug(c);
        bucket_fill(c, x, y + 1);
        bucket_fill(c,x+1,y);
