...
ArrayWithLength base_case(ArrayWithLength arr) {
    ArrayWithLength ret;
    int size = arr.len;
    ret.arr = (uint16_t *) malloc(sizeof(uint16_t) * size);
    ret.len = size;
...
ArrayWithLength combine(ArrayWithLength arr1, ArrayWithLength arr2) {
    /*return array allokation*/
    ArrayWithLength ret;
    int size = arr1.len + arr2.len;
    ret.arr = (uint16_t *)malloc(sizeof(uint16_t) * size);
    ret.len = size;
    /*Initalisierung von Iteratoren*/
    int i = 0;   
    int j = 0; 
    int k = 0; 
    /*combine von den zwei Teil Arrays*/
...
ArrayWithLength merge_k(ArrayWithLength *arrs, size_t count) {
    ...
    /*Die Mitte vom Array finden*/ 
    int middle= count / 2;
    /*Zwei Hilfarrays erstellen und den MergeSort verwenden*/
    ArrayWithLength left = merge_k(&arrs[0], middle);
    ArrayWithLength right = merge_k(&arrs[middle], count - middle);

    /*return Array erstellen und allokieren*/
    ArrayWithLength ret;
